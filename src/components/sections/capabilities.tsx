      ...
      <div className="section-container" style={{ width: "80%", margin: "0 auto", padding: "0 clamp(24px, 5vw, 64px)" }}>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: "clamp(20px, 2.5vw, 24px)" }}
              ...
              <div style={{ marginTop: "24px" }}>
                {["CRM / ERP", "Carrier APIs", "Internal Tools"].map((label) => (
                  <div key={label} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px" }}>
                    ...
                    <span style={{ fontSize: "clamp(13px, 1.1vw, 15px)", fontWeight: 500, color: "#F1F5F9" }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <motion.div variants={fadeUp} style={{
            ...
            <div style={{ padding: "clamp(24px, 2.5vw, 32px)", position: "relative", zIndex: 10, width: "100%", background: "linear-gradient(to top, rgba(10,15,30,0.95) 0%, rgba(10,15,30,0.4) 70%, transparent 100%)" }}>
              <h3 style={{ fontSize: "clamp(1.5rem, 2vw, 1.875rem)", fontWeight: 500, color: "#F1F5F9", letterSpacing: "-0.02em", marginBottom: "16px" }}>
