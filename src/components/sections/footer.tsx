...
export function Footer() {
  return (
    <footer style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "clamp(28px, 4vw, 44px) 0" }}>
      <div className="footer-content" style={{ width: "80%", margin: "0 auto", padding: "0 clamp(24px, 5vw, 64px)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "20px" }}>

        {/* Brand — icon + wordmark */}
        <div className="footer-brand" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <svg width="18" height="18" viewBox="397 94 611 522" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ opacity: 0.5 }}>
          ...
          <span style={{ fontSize: "clamp(14px, 1.1vw, 16px)", fontWeight: 500, color: "#475569", letterSpacing: "-0.01em" }}>StrideShip</span>
        </div>

        {/* Nav links */}
        <div className="footer-links" style={{ display: "flex", gap: "clamp(16px, 2.5vw, 28px)", alignItems: "center", flexWrap: "wrap" }}>
          {[
            ...
            <Link key={href} href={href} style={{ fontSize: "clamp(12px, 1vw, 14px)", color: "#334155", textDecoration: "none", letterSpacing: "0.02em", transition: "color 150ms ease-out" }}
              onMouseOver={(e) => (e.currentTarget.style.color = "#94A3B8")}
              onMouseOut={(e) => (e.currentTarget.style.color = "#334155")}>
              {label}
            </Link>
          ))}
          <a href="https://www.linkedin.com/company/strideship/" target="_blank" rel="noopener noreferrer"
            style={{ fontSize: "clamp(12px, 1vw, 14px)", color: "#334155", textDecoration: "none", letterSpacing: "0.02em", transition: "color 150ms ease-out" }}
            onMouseOver={(e) => (e.currentTarget.style.color = "#94A3B8")}
            onMouseOut={(e) => (e.currentTarget.style.color = "#334155")}>
            LinkedIn
          </a>
        ...
        {/* Copyright */}
        <p style={{ fontSize: "clamp(11px, 0.9vw, 13px)", color: "#1E293B", letterSpacing: "0.04em" }}>
          &copy; 2026 StrideShip
        </p>
      </div>
    </footer>
  );
